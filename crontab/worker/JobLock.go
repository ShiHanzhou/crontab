...
func (jobLock *JobLock) TryLock() (err error) {
	var (
		...
		keepReapChan   <-chan *clientv3.LeaseKeepAliveResponse
		txn            clientv3.Txn
		lockKey        string
	...
	//2、自动续租
	if keepReapChan, err = jobLock.lease.KeepAlive(cancelCtx, leaseId); err != nil {
		goto FAIL
	}
	...
	go func() {
		...
		for {
			select {
			case keepResp = <-keepReapChan: //自动续租的应答
				if keepResp == nil {
					goto END
				}
			}
